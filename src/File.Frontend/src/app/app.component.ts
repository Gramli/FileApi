...
export class AppComponent implements OnInit {
  ...
  protected convert(file: File, fileName: string): void {
    this.ngbModal
      ...
      .closed.subscribe((selectedExtension: string) => {
        this.fileService.convertFile(
          file,
          selectedExtension,
          (fileContent) => {
            saveAs(
              fileContent,
              this.replaceExtension(fileName, selectedExtension)
            );
            this.notifierService.showSuccess(
              `Successfuly converted file: ${fileName}`
            );
          },
          () => {
            this.notifierService.showError(
              `Error to convert file: ${fileName}`
            );
          }
        );
      });
      ...
      .closed.subscribe((selectedExtension: string) => {
        this.fileService.exportFile(
          ...
          (fileContent) => {
            ...
            this.notifierService.showSuccess(
              `Successfuly exported file id: ${id}`
            );
          },
          () => {
  ...
  protected onDownloadFile(id: number): void {
    this.fileService.downloadFile(
      ...
      (fileContent) => {
        ...
        this.notifierService.showSuccess(
          `Successfuly downloaded file id: ${id}`
        );
      },
      () => {
  ...
  protected onDownloadFileAsJson(id: number): void {
    this.fileService.downloadFileAsJson(
      id,
      (fileContent) => {
        this.saveFile(id, fileContent);
        this.notifierService.showSuccess(
          `Successfuly downloaded as json file id: ${id}`
        );
      },
      () => {
        this.notifierService.showError(`Error to download file id: ${id}`);
  ...
  protected onUploadFileSelected(event: any): void {
    ...
    this.fileService.uploadFile(
      ...
      () => {
        this.notifierService.showSuccess(
          `Successfuly uploaded file id: ${file.name}`
        );
      },
      (error) => {
        this.notifierService.showError(`Error: ${error}`);
  ...
  private saveFile(
    ...
    saveAs(
      ...
      extension === undefined
        ? file?.fileName
        : this.replaceExtension(file!.fileName, extension)
    );
  }
